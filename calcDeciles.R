calcDeciles <- function(probabilities, labels) {
  ...
  predicted.sd <- 100* c(sd(dec1.resp),sd(dec2.resp),sd(dec3.resp),sd(dec4.resp),sd(dec5.resp),
  ...
  calibration <- data.frame(predicted, observed, predicted.sd)
  colnames(calibration) <- c('predicted rates', 'observed rates', 'predicted sd')
